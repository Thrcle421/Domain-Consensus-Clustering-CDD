...
class BaseTrainer(object):
    def __init__(self, config,  writer):
        ...
        self.score_vec = None 
        self.test_loader = get_dataset(self.config, self.config.target, self.config.target_classes, batch_size=10, test=True, validate=True)
        if self.config.task=='imagenet-caltech':
            self.src_loader = get_cls_sep_dataset(self.config, self.config.source, self.config.source_classes, batch_size=10, test=True)
        else:
            self.src_loader = get_dataset(self.config, self.config.source, self.config.source_classes, batch_size=10, test=True)
        self.tgt_loader = get_dataset(self.config, self.config.target, self.config.target_classes, batch_size=10, test=True)
        self.best_prec = 0.0
...
#            pred_label = common_index[pred_label]
    ...
    def open_validate(self, i_iter):
        ...
        if uk_index not in accs.avg:
            ...
            self.neptune_metric('memo-val/Test Accuracy[center]', acc)    
            return acc, acc, 0.0, 0.0, 0.0
        bi_rec = metrics.recall_score(gt_binary, pred_binary)
        bi_prec = metrics.precision_score(gt_binary, pred_binary)
        self.neptune_metric('val/bi recall[center]', bi_rec)
        self.neptune_metric('val/bi prec[center]', bi_prec)
