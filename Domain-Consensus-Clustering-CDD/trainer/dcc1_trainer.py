...
class Trainer(BaseTrainer):
    ...
    def iter(self, i_iter):
        ...
        s_batch = next(self.loader_s)
        _, s_batch = s_batch
        s_img, s_label, _= s_batch
        ...
        _, t_neck, _, t_af_softmax = self.model(t_img.cuda())
        cnt=[]
        for i in t_label.unique().tolist():
            cnt.append(i)
        for i in s_label.unique().tolist():
            cnt.append(i)
        t=[]
        s=[]
        cnt=torch.tensor(cnt)
        for i in cnt.unique().tolist():
            msk = (t_label == i).squeeze()
            i_index = msk.nonzero(as_tuple=False)
            if msk.sum()!=0:
                mdl=int(int(msk.sum())/3)
                t_=torch.chunk(t_neck[i_index],chunks=mdl,dim=0)
                for i,chuk in enumerate(t_):
                    t.append(chuk)
            else:
                msk = (s_label == i).squeeze()
                i_index = msk.nonzero(as_tuple=False)
                mdl=int(int(msk.sum())/3)
                s_ = torch.chunk(s_neck[i_index], chunks=mdl, dim=0)
                for i, chuk in enumerate(s_):
                    t.append(chuk)
        for i in cnt.unique().tolist():
            msk = (s_label == i).squeeze()
            i_index = msk.nonzero(as_tuple=False)
            if msk.sum()!=0:
                ...
                for i, chuk in enumerate(t_):
                    s.append(chuk)
        a1,b1,c1=t[0].size()
        s=torch.stack(s).view(-1,c1)
        t=torch.stack(t).view(-1,c1)
        counter = torch.ones(self.config.num_sample) * self.config.num_pclass
        counter = counter.long().tolist()
        cdd_loss = self.cdd.forward([s], [t], counter, counter,len(cnt.unique().tolist())/len(s_label.unique().tolist()),len(cnt.unique().tolist())/len(t_label.unique().tolist()))['cdd']
    ...
    def train(self):
        ...
        if not self.warmup:
            t_centers, plabel_dict, class_set ,s_newset,t_newset= self.cluster_matching(0)
            ...
            self.loader = init_class_dataset(self.config, plabel_dict=plabel_dict, src_class_set=s_newset, tgt_class_set=t_newset, length=self.config.stage_size)
            self.loader_s=init_class_dataset_s(self.config, plabel_dict=plabel_dict, src_class_set=s_newset, tgt_class_set=t_newset, length=self.config.stage_size)
    ...
    def re_clustering(self, i_iter):
        t_centers, plabel_dict, class_set,s_newset,t_newset = self.cluster_matching(i_iter)
        self.memory.init(t_centers)
    ...
    def target_filtering(self, t_feats, t_codes, t_gts, s_feats, s_codes, s_centers, t_centers, cycle_pair):
        ...
        freq = {}
        cycle_pair_clone=cycle_pair.copy()
        for i in range(n_tgt):
            flag=0
            for s_index,t_index in cycle_pair_clone:
                if t_index==i:
                    flag=1
            if flag==0:
                cycle_pair_clone.append((i+n_src,i))
        s_newset=[i for i in range(n_src)]
        t_newset=[]
        for s_index, t_index in cycle_pair_clone:
            t_newset.append(s_index)
        ...
        return filtered_cluster_label, filtered_pair, freq,s_newset,t_newset
    ...
    def cluster_matching(self, step):
        ...
        filtered_cluster_label, filtered_pair, freq,s_newset,t_newset = self.target_filtering(t_feats, t_codes, gt_dict, s_feats, s_labels, s_centers, t_centers, inter)
        ...
        return t_centers, filtered_cluster_label, label_set,s_newset,t_newset
