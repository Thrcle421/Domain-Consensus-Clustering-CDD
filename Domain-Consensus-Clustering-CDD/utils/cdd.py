...
class CDD(object):
    ...
    def forward(self, source, target, nums_S, nums_T,s,t):
        assert(len(nums_S) == len(nums_T)), \
        ...
        mmds = kernel_dist_ss/int(s) + kernel_dist_tt/int(t) - 2 * kernel_dist_st
    ...
    def target_only_forward(self, target, nums_T):
        ...
        mmds = kernel_dist_tt 
        intra_mmds = torch.diag(mmds, 0)
        intra = torch.sum(intra_mmds) / (2*self.num_classes)
