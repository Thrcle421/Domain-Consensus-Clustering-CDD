...
from .class_dataset import ClassAwareDataset,ClassAwareDataset_s
...
def get_dataset(config, dataset, class_set, label_list=None, test=False, batch_size=None, plabel_dict=None, get_loader=True, length=None, binary_label=None, class_wise=False, validate=False):
    ...
    list_path = '/opt/data/private/Domain-Consensus-Clustering-CDD/dataset/list/{}/{}.txt'.format(config.task,dataset)
...
def init_class_dataset(config, plabel_dict, src_class_set, tgt_class_set, length=None, uk_list=None):
    ...
    src_list_path = '/opt/data/private/Domain-Consensus-Clustering-CDD/dataset/list/{}/{}.txt'.format(config.task, config.source)
    ...
    dataset = ClassAwareDataset(root_path, config.num_pclass, transform,  tgt_class_set, plabel_dict, num_steps=length*config.num_sample)
...
def init_class_dataset_s(config, plabel_dict, src_class_set, tgt_class_set, length=None, uk_list=None):
    transform = get_transform()
    src_list_path = '/opt/data/private/Domain-Consensus-Clustering-CDD/dataset/list/{}/{}.txt'.format(config.task, config.source)
    if config.task =='visda' or config.task=='imagenet-caltech':
        root_path = osp.join(config.root[config.task], config.source)
    ...
    dataset = ClassAwareDataset_s(root_path, config.num_pclass, transform,  src_class_set, src_list_path, num_steps=length*config.num_sample)
    dataloader = DataLoader(dataset=dataset, batch_size=config.num_sample, num_workers=config.num_workers, shuffle=True, drop_last=True)
    dataloader = enumerate(dataloader)
    return dataloader
